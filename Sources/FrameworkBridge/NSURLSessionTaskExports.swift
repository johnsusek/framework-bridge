...
@objc(URLSessionTask) protocol URLSessionTaskExports: JSExport, NSObjectExports {
  ...
  */
  ...
  @objc func sendMessage(message: String, completionHandler: JSValue)
...
extension URLSessionTask: URLSessionTaskExports {
  @objc public func sendMessage(message: String, completionHandler: JSValue) -> Void {
      guard let self = self as? URLSessionWebSocketTask else { return }
      self.send(URLSessionWebSocketTask.Message.string(message), completionHandler: { p1 in
